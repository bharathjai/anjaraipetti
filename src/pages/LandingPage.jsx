    ...
    <section ref={sectionRef} className="mx-auto w-full max-w-7xl px-6 pb-20 pt-12 md:px-10 md:pt-20">
      <div className="grid items-center gap-14 lg:grid-cols-[1fr_1.1fr]">
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
          <p className="text-xs uppercase tracking-[0.35em] text-cocoa/70">Tamil Heritage Spice House</p>
          ...
          <p className="mt-7 max-w-xl text-lg leading-relaxed text-truffle/80">
            Anjaraipetti makes bold cooking masalas with authentic South Indian flavor and restaurant-style finish.
          </p>
          <div className="mt-8 flex flex-wrap gap-4">
            <Link
              to={`/product/${heroProduct.id}`}
              className="rounded-full bg-truffle px-7 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-porcelain transition hover:bg-espresso"
            >
              View Products
            </Link>
            <Link
              to="/cart"
              className="rounded-full border border-truffle/25 bg-white/80 px-7 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-truffle transition hover:bg-almond"
            >
              Open Cart
            </Link>
          </div>
        ...
        >
          ...
          <div className="relative overflow-hidden rounded-[2rem] border border-truffle/10 bg-white/70 p-4 shadow-luxe backdrop-blur-xl">
            <img src={heroProduct.image} alt={heroProduct.name} className="aspect-[4/5] w-full rounded-[1.5rem] object-cover" />
            <div className="mt-4 rounded-2xl border border-truffle/10 bg-white/70 p-4">
              <h2 className="font-display text-3xl text-espresso">{heroProduct.name}</h2>
              <p className="mt-1 text-sm uppercase tracking-[0.2em] text-cocoa/70">{heroProduct.subtitle}</p>
            </div>
          </div>
        </motion.div>
      </div>
          ...
          {products.map((product, index) => (
            ...
            >
              ...
              <Link
                ...
                className="mt-4 inline-flex rounded-full border border-truffle/20 bg-white px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-truffle"
              >
                View
              </Link>
            </motion.article>
